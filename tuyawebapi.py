...
TUYA_PASSWORD_INNER_CIPHER = Cipher(
  ...
  modes.CBC(
    bytearray(
      [119, 36, 86, 242, 167, 102, 76, 243, 57, 44, 53, 151, 233, 62, 87,
       71])),
...
class TuyaAPISession:
  ...
  def __init__(self, username, country_code):
    ...
    self.country_code = "EU" if not country_code else country_code
  ...
  def get_signature(query_params: dict, encoded_post_data: str):
    ...
    return hmac.HMAC(key=EUFY_HMAC_KEY,
  ...
  def _request(
    ...
    resp = self.session.post(
      ...
      params={
        **query_params,
        "sign":
        self.get_signature(query_params, encoded_post_data),
      },
  ...
  def request_session(self, username, country_code):
    ...
    print("        TUYA country code: {}".format(country_code))
  ...
  def acquire_session(self):
    ...
    self.session_id = self.default_query_params["sid"] = session_response[
      "sid"]
    self.base_url = session_response["domain"]["mobileApiUrl"]
    self.country_code = session_response["domain"]["regionCode"]
